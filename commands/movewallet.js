...
module.exports = {
	commands: 'movewallet',
	description: 'Move currency to your wallet',
  async execute(message, arguments, text) {
      ...
      if(amount > bankBalance) return message.channel.send("You don't have enough money!");

      await db.set(`wallet_${message.author.id}`, currentBalance + amount);
      await db.set(`bank_${message.author.id}`, bankBalance - amount);

      message.channel.send(`Moved ${amount.toLocaleString()}${currency} to your wallet.`);
  }
}